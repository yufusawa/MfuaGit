...
#### Почему не облако типа Yandex Диск или Google Диск?
...
1. **Инициализация** - ```git status``` - показать состояние текущего репозитория
2. **Подготовка** (добавлены изменения) - ```git add .``` - добавление изменений
3. **Фиксаци**я, сохранение - ```git commit -m 'Update README.md - fix'``` - сохранение с комментарием
